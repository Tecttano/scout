#!/usr/bin/env python3
import subprocess, argparse, ipaddress, re, json, urllib.request, ssl, socket
...
def run_dns(target):
    ...
    try:
        ...
        if not a_result.startswith("[!]"):
            ...
            dns_info.append(f"A Record Lookup Error: {a_result}")
            dns_json["a_record_error"] = a_result
    except Exception as e:
        dns_info.append(f"A Record Lookup Error: {str(e)}")
        dns_json["a_record_error"] = str(e)
    
    try:
        ns_result = run_cmd(["dig", target, "NS", "+short"], "")
        if not ns_result.startswith("[!]"):
            ...
            dns_info.append(f"NS Lookup Error: {ns_result}")
            dns_json["ns_error"] = ns_result
    except Exception as e:
        dns_info.append(f"NS Lookup Error: {str(e)}")
        dns_json["ns_error"] = str(e)
    
    if (args.mx or args.dns_info) and not is_ip(target):
        try:
            mx_result = run_cmd(["dig", target, "MX", "+short"], "")
            if not mx_result.startswith("[!]"):
            ...
            dns_info.append(f"MX Lookup Error: {str(e)}")
            dns_json["mx_error"] = str(e)
    
    if (args.txt or args.dns_info) and not is_ip(target):
        try:
            txt_result = run_cmd(["dig", target, "TXT", "+short"], "")
            if not txt_result.startswith("[!]"):
                txt_records = []
                spf_records = []
            ...
            dns_info.append(f"TXT Lookup Error: {str(e)}")
            dns_json["txt_error"] = str(e)
    
    if (args.soa or args.dns_info) and not is_ip(target):
        try:
            soa_result = run_cmd(["dig", target, "SOA", "+short"], "")
            if not soa_result.startswith("[!]"):
            ...
            dns_info.append(f"SOA Lookup Error: {str(e)}")
            dns_json["soa_error"] = str(e)
    
    if (args.cname or args.dns_info) and not is_ip(target):
        try:
            cname_result = run_cmd(["dig", target, "CNAME", "+short"], "")
            if not cname_result.startswith("[!]") and cname_result.strip():
            ...
            dns_info.append(f"CNAME Lookup Error: {str(e)}")
            dns_json["cname_error"] = str(e)
    
    if (args.aaaa or args.dns_info) and not is_ip(target):
        try:
            aaaa_result = run_cmd(["dig", target, "AAAA", "+short"], "")
            if not aaaa_result.startswith("[!]"):
            ...
            dns_info.append(f"AAAA Lookup Error: {str(e)}")
            dns_json["aaaa_error"] = str(e)
...
def run_ping(target, count=4):
    try:
        result = run_cmd(["ping", "-c", str(count), target], "")
